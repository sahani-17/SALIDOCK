...
export function useDockingWorkflow({ isBlind = false } = {}) {
    ...
    const handleSmilesSubmit = async () => {
        ...
        try {
            ...
            setError(err?.message || 'Failed to generate ligand from SMILES');
            toast.error(err?.message || 'SMILES conversion failed');
        } finally {
            setLoading(false);
            setLoadingMessage('');
        }
    ...
    const handleSequencePrediction = async () => {
        ...
        try {
            ...
            toast.error(err?.message || 'Sequence prediction failed');
        } finally {
            setLoading(false);
            setLoadingMessage('');
        }
    ...
    const handleUniProtFetch = async () => {
        ...
        try {
            ...
            toast.error(err?.message || 'UniProt fetch failed');
        } finally {
            setLoading(false);
            setLoadingMessage('');
        }
    ...
    const handleProteinPreparation = async () => {
        ...
        try {
            ...
            toast.error(err?.message || 'Protein preparation failed');
        } finally {
            setLoading(false);
            setLoadingMessage('');
        }
