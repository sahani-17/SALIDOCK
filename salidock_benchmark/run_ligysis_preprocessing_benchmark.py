...
=======================================
LIGYSIS-2024 blind validation — measures preprocessed tool & consensus performance.

Runs ONLY in preprocessed mode (which matches our production pipeline)
to cut the benchmark runtime in half (saving ~9 hours of CPU time).
"""
...
def preprocess_pdb(input_pdb: str, output_pdb: str) -> dict:
    ...
    lines_in = Path(input_pdb).read_text(encoding='utf-8', errors='ignore').splitlines(keepends=True)
...
def wrrf_top_n(
    ...
    weights: dict, k: int = 60, radius: float = 6.0, top_n: int = 5
) -> List[np.ndarray]:
    """Compute wRRF top-N centres."""
    all_pockets = []
    ...
    return [c['center'] for c in clusters[:top_n]]
...
def main():
    print('=' * 90)
    print('  LIGYSIS-2024 BLIND VALIDATION BENCHMARK (PREPROCESSED ONLY)')
    print('=' * 90)
    ...
    parser.add_argument('--limit', type=int, default=None, help='Limit evaluation to first N chains')
    ...
    if args.limit is not None:
        ...
        print(f"[INFO] Limited evaluation to first {args.limit} chains.")
    ...
    print(f"Optimal Weights: fpocket={w['fpocket']:.4f}, p2rank={w['p2rank']:.4f}, puresnet={w['puresnet']:.4f}\n")
    ...
    ckpt_prep: dict = {}
    if ckpt_prep_path.exists():
        try:
            ckpt_prep.update(json.loads(ckpt_prep_path.read_text(encoding='utf-8')))
        except Exception:
            pass

    print(f"[INFO] Checkpoints: preprocessed={len(ckpt_prep)} entries loaded")
    ...
    for tool in tools_list:
        ...
        tsv_path = tsv_dir / f"LIGYSIS_preprocessed_{tool}.tsv"
    ...
    for idx, target_chain in enumerate(chains, 1):
        ...
        remaining_chains = total - idx
        eta_sec = avg_time * remaining_chains
        ...
        elapsed_str = f"{elapsed_h:02d}h {elapsed_m:02d}m {elapsed_s:02d}s"

        print(f"[{idx}/{total}] {target_chain} | Elapsed: {elapsed_str} | ETA: {eta_str}", flush=True)
        ...
        # ── Preprocessed isolated chain prediction ───────────────────────────
        ...
        try:
            ...
            print(f"  [ERROR] Prediction sweep failed: {exc}")
            continue
    ...
    for tool in tools_list:
        ...
        with open(tsv_path, encoding='utf-8') as fh:
            for line in fh.readlines()[1:]:
                ...
                if len(parts) < 9:
                    continue
                pid, t, rank = parts[0], parts[2], int(parts[3])
                px, py, pz = float(parts[4]), float(parts[5]), float(parts[6])
                dist = float(parts[8])
                results_map[pid][t].append({
                    'rank': rank,
    ...
    counters: dict = defaultdict(lambda: {'top1': 0, 'top5': 0, 'n': 0, 'ok_vec': []})
    ...
    for pid in all_chains_seen:
        ...
        for tool in tools_list:
            ...
            for p in preds:
                ...
                if d <= DCA_THRESHOLD:
                    ...
                    if p['rank'] <= 5:
                        top5 = True
            counters[tool]['top1'] += int(top1)
            counters[tool]['top5'] += int(top5)
            counters[tool]['n']    += 1
            counters[tool]['ok_vec'].append(int(top1))

        # Consensus top-5
        fp_preds  = results_map[pid]['fpocket']
        p2r_preds = results_map[pid]['p2rank']
        pur_preds = results_map[pid]['puresnet']
        ...
        c_preds = wrrf_top_n(fp_l, p2r_l, pur_l, w, top_n=5)
        c1_ok = c5_ok = 0
        for rank_idx, c_pred in enumerate(c_preds):
            md = min(euclidean_distance(c_pred, tc) for tc in chain_true)
            if md <= DCA_THRESHOLD:
                if rank_idx == 0:
                    c1_ok = 1
                c5_ok = 1
        counters['consensus']['top1'] += c1_ok
        counters['consensus']['top5'] += c5_ok
        counters['consensus']['n']    += 1
        counters['consensus']['ok_vec'].append(c1_ok)
    ...
    hdr = (
        f"\n{'='*90}\n"
        f"{'LIGYSIS-2024 VALIDATION BENCHMARK RESULTS (PREPROCESSED)':^90}\n"
        f"{'='*90}\n"
        f"DCA threshold: {DCA_THRESHOLD} Å | N_chains={len(all_chains_seen)}\n"
        f"Optimal Weights: fpocket={w['fpocket']:.4f} p2rank={w['p2rank']:.4f} puresnet={w['puresnet']:.4f}\n"
        f"{'─'*90}"
    )
    report_lines.append(hdr)
    print(hdr)

    col = f"{'Tool / Method':<25} | {'DCA@top1':>10} | {'DCA@top5':>10} | {'N':>6} | {'95% CI top1'}"
    ...
    for tool in tools_all:
        cnt = counters[tool]
        ci_lo, ci_hi = wilson_ci(cnt['top1'], cnt['n'])

        row = f"  {tool:<25} | {pct(cnt['top1'],cnt['n']):>10} | {pct(cnt['top5'],cnt['n']):>10} | {cnt['n']:>6} | [{ci_lo:.1f}%–{ci_hi:.1f}%]"
    ...
    try:
        ...
        fig.suptitle('LIGYSIS-2024 Blind Validation\nDCA success rate (N={})'.format(len(all_chains_seen)), fontsize=12, fontweight='bold')
        ...
        top1_vals = [counters[t]['top1'] / counters[t]['n'] * 100 if counters[t]['n'] > 0 else 0 for t in tools_all]
        top5_vals = [counters[t]['top5'] / counters[t]['n'] * 100 if counters[t]['n'] > 0 else 0 for t in tools_all]

        ax.bar(x - width/2, top1_vals, width, label='DCA@top1', color='#3b82f6', edgecolor='black', linewidth=0.5)
        ax.bar(x + width/2, top5_vals, width, label='DCA@top5', color='#10b981', edgecolor='black', linewidth=0.5)

        for i, (t1, t5) in enumerate(zip(top1_vals, top5_vals)):
            ax.text(x[i] - width/2, t1 + 0.5, f'{t1:.1f}%', ha='center', va='bottom', fontsize=8, color='#1e3a8a', fontweight='bold')
            ax.text(x[i] + width/2, t5 + 0.5, f'{t5:.1f}%', ha='center', va='bottom', fontsize=8, color='#064e3b', fontweight='bold')
